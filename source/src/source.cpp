...
int main() {
	try {
		...
		for (auto e : empleados) {
			...
			std::cout << "| " << e.id << " | " << std::left << std::setw(24)
			          << apellidos << generarEspacios(apellidos) << " | "
			          << std::setw(16) << e.nombre << generarEspacios(e.nombre)
			          << " | " << std::right << std::setw(14) << e.salarioBruto
